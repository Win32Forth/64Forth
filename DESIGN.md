...
**Public domain.**  
**Updated:** 2026-09-22 — **v1.4.2** (build 41) + post-release in tree: GRAPHICS color depths + Sample IMAGEVIEW64/EDIT64; cold **`Kernel/xref.fth`** (`REF`/`XREF`/`ANYWORDS`, VIEW leaf:line; KEY during console eval). Prior release: ANS SMUDGE / hide-until-`;`; FILE-ECHO line#s; GRAPHICS mouse + Sample/DOODLE64; Emitter SA CONSTANT/VALUE/LIT reloc for `EMIT-WINDOW-APP`; **1.4.1** TRAVERSE/DBG VIEW+HL + cold boot transcript + INCLUDE `file:line`; **1.4.0** token maps + pause UI; **1.3.9** FLOAD/INCLUDED load-cwd + THROW/CATCH polish; **1.3.8** SZ-EDITOR own window; **1.3.7** ANEW/MARKER + BREAK/BPGO; **1.3.6** Emitter **0.7** SA locals/BI + window I/O remap; app-kit freeze still `Docs/APPKIT.md` (GRAPHICS 80×25 / 640×400; `\ANS`/`\TCOM`/`\EMITTER`; tetra + PIMAIN).
