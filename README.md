...
- [x] v0.6: FIND-before-number, Tools `?`, CODE `DEPTH`, include buffer 256 KiB, Hayes harness under `src/Harness/`  
- [x] String Ext, Locals Ext (`(LOCAL)` / `LOCALS|`), Facility + **Facility Ext** (`EKEY>FKEY`, `K-*`, structures)  
- [x] Extended Character (UTF-8 XChar)  
- [x] Modular Library ANS-VALIDATE: `ANSValidate/ANS-VALIDATE.fth` (~351 passed, 0 failed)  

Optional later: SZ-EDITOR polish, line-at-a-time INCLUDE via fileid, App Sandbox for store builds.
