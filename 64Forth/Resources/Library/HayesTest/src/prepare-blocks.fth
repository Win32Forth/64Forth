...
\
\ Stack notes:
\   OPEN-BLOCK-FILE   ( c-addr u -- bid ior )
...
: PREPARE-HAYES-BLOCKS ( -- )
   ...
   DUP 0= IF                            \ open succeeded (ior = 0)
      ...
      NIP NIP                           \ ( bid )
      USE-BLOCK-FILE
      .( prepare-blocks: using Application Support/64Forth/hayes-blocks.blk ) CR
   ELSE                                 \ open failed
      ...
      DUP 0= IF
         DROP                           \ ( bid )
         USE-BLOCK-FILE
         .( prepare-blocks: created Application Support/64Forth/hayes-blocks.blk ) CR
      ELSE
