...
\ Bookmark / restore dictionary so each row's :NONAME can be discarded.
\ (MARKER name cannot be used inside a colon def — the name is needed at compile time.)
variable row-here
variable row-latest
: row-save    ( -- )  here row-here !  latest @ row-latest ! ;
: row-restore ( -- )  row-latest @ latest !  row-here @ here - allot ;
...
: mainx  ( -- )
  ...
  imr ima mat-byte-size mybounds do
    ...
    cell +loop
    nip
    row-restore   \ drop this row's generated code (else MAIN fills the dictionary)
  row-size cells +loop
  drop
;
