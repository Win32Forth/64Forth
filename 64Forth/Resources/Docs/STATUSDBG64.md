...
**64Forth ITC `DEBUG` / `DBG`:** **1.3.0+** (current **1.3.4**) — help column, LIT/branch ±CELLS inline, pause spacing, ok> focus; **1.2.0** source-alias highlight + F8/Esc/⌘Q. Interactive `CODE`/`END-CODE`: see `Assembler/ASSEMBLER.fth` / `STATUSASM64.md`. Emitter (in progress): `Library/Emitter/`. SYSVOC holds `DBG-SHOW-XT` / Hyper debug hooks.
