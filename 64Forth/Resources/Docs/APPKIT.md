...
3. **Stand-alone packaging (in progress):**
   ...
   - **Done (Forth entry):** `EMIT-APP ( xt -- )` / `EMIT-APP-TO ( xt c-addr u -- )` in **FORTH** (`Library/Emitter/app.fth`) — `/EMIT-STANDALONE` + `/EMIT-UNBOUND` + `TGT-BUILD` + `SAVE-IMAGE` + `SYSTEM` `app-build.sh`. App basename = word name. Example: `' MAIN EMIT-APP` → `./MAIN.app`. Smoke: `Library/Emitter/EmitterSmoke/emit-app-smoke.fth`.
